...
# python3 ops/build_client_portal.py [config.json] [out/index.html]
_a = sys.argv[1:]
CFG = ROOT / (_a[0] if len(_a) > 0 else "ops/clients.json")
OUT = ROOT / (_a[1] if len(_a) > 1 else "clients/index.html")
...
def main():
    ...
    for c in cfg["clients"]:
        ...
        badge = ('<span class="logo"><img src="%s%s" alt="%s logo"></span>'
                 % (base, c["logo"], html.unescape(c["name"]))) if c.get("logo") else ""
    ...
    page = TEMPLATE.replace("__HEADING__", cfg.get("heading", "See what your team built")) \
                   .replace("__LEDE__", cfg.get("lede",
                       "Each organisation below has a chatbot built for it by a team at the "
                       "ECCU GenAI &amp; Python Camp 2026. Your bot, its code, your profile "
                       "and your podcast are locked until you enter your password.")) \
                   .replace("__STEP3__", cfg.get("step3", "Use the password the camp team sent you.")) \
                   .replace("__NOTE__", cfg.get("note",
                       "<b>A note on the lock.</b> Each card's links are encrypted with its own "
                       "password. Nothing exists on this page until the correct password is "
                       "entered, so a stray visitor sees nothing.")) \
